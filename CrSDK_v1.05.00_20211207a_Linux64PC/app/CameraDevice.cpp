...
namespace cli
{
    CameraDevice::CameraDevice(std::int32_t no, CRLibInterface const *cr_lib, SCRSDK::ICrCameraObjectInfo const *camera_info)
        : m_cr_lib(cr_lib), m_number(no), m_device_handle(0), m_connected(false), m_conn_type(ConnectionType::UNKNOWN), m_net_info(), m_usb_info(), m_prop(), m_lvEnbSet(true), m_modeSDK(SCRSDK::CrSdkControlMode_ContentsTransfer), m_spontaneous_disconnection(false)
    {
    ...
    void CameraDevice::set_select_media_format()
    {
            ...
            (SDK::CrMediaFormat::CrMediaFormat_Disable == m_prop.media_slot2_full_format_enable_status.current))
        {
            // Not a settable property
            tout << std::endl
                 << "Slot1 and Slot2 is can not format\n";
            return;
        }
        ...
        text input;
        tout << std::endl
             << "Would you like to format the media? (y/n):" << std::endl;
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
            tout << "Skip format.\n";
        ...
        while (true)
        {
            ...
            if (prop_list && 1 == nprop)
            {
                ...
                if (getCodes == prop.GetCode())
                {
                    ...
                    tout << "\r"
                         << "FormatProgressRate:" << prop.GetCurrentValue();
                }
            }
    ...
    void CameraDevice::set_custom_wb()
    {
        ...
        tout << std::endl
             << "Set the Exposure Program mode to P mode";
